...
private:
    ...
    // 确保 boot_button_ 的构造函数参数能够正确识别长按和单击
    // BOOT_BUTTON_GPIO, false (低电平有效), 3000ms (长按阈值), 0ms (短按阈值)
    Button boot_button_; 
    LcdDisplay* display_;
    ...
    void InitializePowerSaveTimer() {
        ...
        power_save_timer_->OnEnterSleepMode([this]() {
            ...
            #endif
            //gpio_set_level(SLEEP_GOIO, 0); // 示例：进入睡眠模式时可能需要调整GPIO
        });
        power_save_timer_->OnExitSleepMode([this]() {
            ...
            display->SetEmotion("neutral");
                ...
                GetBacklight()->RestoreBrightness();
            #endif
            //gpio_set_level(SLEEP_GOIO, 1); // 示例：退出睡眠模式时可能需要调整GPIO
        });
        power_save_timer_->OnShutdownRequest([this]() {
            //pmic_->PowerOff(); // 如果有 PMIC，在这里调用关机
            ESP_LOGI(TAG,"Not used for a long time. Shut down. Press and hold to turn on!");
            gpio_set_level(SLEEP_GOIO, 0); // 假设 SLEEP_GOIO 为0是关机信号
        });
    ...
    void InitializeButtons() {
        ...
        boot_button_.OnClick([this]() {
            auto& app = Application::GetInstance(); // 先获取 app 实例，方便所有逻辑使用

            // **核心修复：** 短按的主要功能（如切换聊天状态）应该总是执行
            app.ToggleChatState(); 

            // **配网逻辑：** 只有在满足所有条件时才触发
            // 条件1: 系统已过保护期
            // 条件2: 设备处于启动状态 (kDeviceStateStarting)
            // 条件3: Wifi 尚未连接 (!WifiStation::GetInstance().IsConnected())
            if (((esp_timer_get_time() / 1000) >= WIFI_CONFIG_PROTECTION_TIME_MS) && // 检查是否已过保护期
                (app.GetDeviceState() == kDeviceStateStarting && !WifiStation::GetInstance().IsConnected())) {
                
                ESP_LOGI(TAG, "单击触发重置Wifi配置 (已通过保护期检查)");
                ResetWifiConfiguration();
            } else {
                // 如果短按没有触发配网，可以打印更详细的日志，便于调试
                ESP_LOGD(TAG, "单击未触发配网。当前时间: %lldms, 保护期: %lldms, 设备状态: %d, WiFi连接: %d", 
                         esp_timer_get_time() / 1000, WIFI_CONFIG_PROTECTION_TIME_MS, 
                         app.GetDeviceState(), WifiStation::GetInstance().IsConnected());
            }
        });
        
        // OnPressRepeat 回调：仅在保护期外触发配网
        boot_button_.OnPressRepeat([this](uint16_t count) {
            ...
            if(count >= 3){
                ESP_LOGI(TAG, "检测到多次重复按键，触发重新配网");
                ResetWifiConfiguration();
            }
        });

        #if (defined(CONFIG_VB6824_OTA_SUPPORT) && CONFIG_VB6824_OTA_SUPPORT == 1)
        // OnDoubleClick 回调：在启动后的一段较长时间内避免误触发OTA
        boot_button_.OnDoubleClick([this]() {
            if ((esp_timer_get_time() / 1000) > (20 * 1000)) { // 20秒后不再通过双击进入OTA
                ESP_LOGI(TAG, "系统启动已超过20秒，忽略OTA双击。(当前时间: %lldms)", (esp_timer_get_time() / 1000));
                return; 
            }
            ESP_LOGI(TAG, "双击触发OTA更新");
            audio_codec.OtaStart(0);
        });
        #endif

        // 长按关机逻辑：不受保护期限制，因为是明确的关机意图
        boot_button_.OnLongPress([this]() {
            ESP_LOGI(TAG, "电源键长按！即将关机...");
            auto display = GetDisplay();
            if (display) {
                display->SetChatMessage("system", "正在关机...");
                vTaskDelay(pdMS_TO_TICKS(1000)); // 延迟1秒显示关机信息
            }
            gpio_set_level(SLEEP_GOIO, 0); // 假设 SLEEP_GOIO 拉低表示关机
            ESP_LOGI(TAG, "电源已切断。");
        });
    }
    ...
    CompactWifiBoardLCD():
        ...
        audio_codec.OnWakeUp([this](const std::string& command) {
            if (command == std::string(vb6824_get_wakeup_word())){
                ...
                ESP_LOGI(TAG,"语音命令触发配网");
                ResetWifiConfiguration();
            }
        });
        
        // 在所有主要初始化完成后，记录系统启动时间
        // esp_timer_get_time() 返回微秒，除以1000转换为毫秒
        startup_time_ms_ = esp_timer_get_time() / 1000; 
    }
